require_relative '../pulse_propagation.rb'
...
RSpec.describe "PulsePropagationTest", type: :request do

  xdescribe 'the overall pulse propogator' do
    let(:pp1) { PulsePropagation.new('./spec/test_input/day20-1.txt') }
    let(:pp2) { PulsePropagation.new('./spec/test_input/day20-2.txt') }

    before(:each) do
      pp1.broadcast_module.class.reset_counts
      pp2.broadcast_module.class.reset_counts
    end

    it 'to create the broadcaster module' do
      expect(pp1.broadcast_module.name).to eq('broadcaster')
      expect(pp1.broadcast_module.destinations.size).to eq(3)
    end

    it 'counts the transitions for example 1' do
      pp1.push_button
      expect(pp1.broadcast_module.class.counts).to eq({low: 8, high: 4})
      expect(pp1.broadcast_module.class.total).to eq(32)
    end

    it 'counts the transitions for example 2' do
      1000.times { pp2.push_button }
      expect(pp2.broadcast_module.class.counts).to eq({low: 4250, high: 2750})
      expect(pp2.broadcast_module.class.total).to eq(11687500)
    end
  end

  xdescribe 'the flip-flop module' do
    let(:ff_off) { FlipFlopModule.new('a', ['b']) }

    it 'ignores a high pulse' do
      expect(ff_off.state).to eq('off')
      ff_off.process_pulse(ff_off.name, 'high')
      ff_off.process_pulse(ff_off.name, 'high')
      expect(ff_off.state).to eq('off')
    end

    it 'flip flops on a low pulse' do
      expected_results = ['off', 'on', 'off', 'on', 'off']

      expected_results.each do |expected_result|
        expect(ff_off.state).to eq(expected_result)
        ff_off.process_pulse(ff_off.name, :low)
      end
    end
  end
  ...
  xdescribe 'Part 1' do
    let(:pp) { PulsePropagation.new }

    before(:each) do
      pp.broadcast_module.class.reset_counts
    end

    it 'finds the correct total' do
      # binding.pry
      1000.times { pp.push_button }
      expect(pp.broadcast_module.class.total).to be > 11687500
      expect(pp.broadcast_module.class.total).to be < 1228811724
      expect(pp.broadcast_module.class.total).to eq(949764474)
    end
  end
  ...
  describe 'Part 2' do
    ...
    it 'finds the minimum number of button pushes to deliver a low pulse to rx' do
      button_pushes = 0
      loop do
        button_pushes += 1
        pp.push_button
        puts "Button pushes: #{button_pushes}"
      end
    end
  end
end
